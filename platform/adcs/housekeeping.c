...
SAT_returnState hk_report_parameters(HK_struct_id sid, tc_tm_pkt *pkt) {
    ...
    if(sid == EX_HEALTH_REP) {
        ...
        cnvF_8(adcs_sensors.gyro_sensor.gyr[0], &pkt->data[5]);
        cnvF_8(adcs_sensors.gyro_sensor.gyr[1], &pkt->data[9]);
        cnvF_8(adcs_sensors.gyro_sensor.gyr[2], &pkt->data[13]);
        ...
        cnv16_8((uint16_t)(40.0/0.01), &pkt->data[0]); // Roll in deg
        cnv16_8((uint16_t)(-45.0/0.01), &pkt->data[2]); // Pitch in deg
        cnv16_8((uint16_t)(90.0/0.01), &pkt->data[4]); // Yaw in deg
        cnv16_8((uint16_t)(1.0/0.001), &pkt->data[6]); // Roll Dot in deg/sec
        cnv16_8((uint16_t)(-2.0/0.001), &pkt->data[8]); // Pitch Dot in deg/sec
        cnv16_8((uint16_t)(3.0/0.001), &pkt->data[10]); // Yaw Dot in deg/sec
        cnv16_8((uint16_t)(6700.0/0.5), &pkt->data[12]); // X ECI in km
        cnv16_8((uint16_t)(6800.0/0.5), &pkt->data[14]); // Y ECI in km
        cnv16_8((uint16_t)(6900.0/0.5), &pkt->data[16]); // Z ECI in km
        pkt->len = 18;
    }
